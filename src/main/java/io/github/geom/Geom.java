...
public class Geom {
   ...
   * @param ord Even number of ordinates forming coordinates for the line string.
   */
  public static LineString lineString(Point... ord) {
    int len = ord.length;
    Coordinate[] cstack = new Coordinate[len];
    for (int i = 0; i < len; i++) {
      cstack[i] = ord[i].getCoordinate();
    }
    return factory.createLineString(cstack);
  }
   ...
   * @param ord Even number of ordinates forming coordinates for the outer ring of the polygon.
   */
  public static Polygon polygon(Point... ord) {
    int len = ord.length;
    Coordinate[] cstack = new Coordinate[len];
    for (int i = 0; i < len; i++) {
      cstack[i] = ord[i].getCoordinate();
    }
    return factory.createPolygon(cstack);
  }
  ...
  /**
   * Retypes (ie. narrows) a geometry collection if possible. <p> If the geometry contains a single object it is
   * narrowed to that object. If the geometry collection is homogeneous it is narrowed to the appropriate sub collection
   * type. Otherwise the collection is returned as is. </p>
   *
  ...
  public static <T extends Geometry> T narrow(GeometryCollection gc) {
    ...
    List<Geometry> objects = new ArrayList<Geometry>(gc.getNumGeometries());
  ...
  /**
   * Converts a geometry object into the associated geometry collection. For example Polygon to MultiPolygon. <p> If the
   * input is already a collection it is returned as is. </p>
   */
  ...
  static List<GeomParser> PARSERS = new ArrayList<GeomParser>();
  ...
  static {
    ...
    PARSERS.add(new GeomParser() {
      @Override
      public Geometry parse(String str) throws Exception {
        return GeoJSON.parse(str, Geometry.class);
      }
    });
  }

  /**
   * Parses the specified string into a geometry. <p> Currently the following geometry formats are supported. <ul>
   * <li>Well Known Text</li> <li>GeoJSON</li> </ul> </p>
   *
